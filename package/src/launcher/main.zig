...
pub fn main() !void {
    ...
    // Child.env_map is a borrowed pointer used later by spawn(). Keep storage
    // in function scope for platforms that need overrides. On macOS, leave it
    // null so Child inherits the environment without copying an EnvMap.
